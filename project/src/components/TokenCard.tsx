...
const TokenCard: React.FC<TokenCardProps> = ({ token }) => {
  ...
  const getProgress = () => {
    ...
    const baseProgress = token.stats.priceChange5m ? Math.min(Math.max(parseFloat(token.stats.priceChange5m), 0), 100) : 0;
            ...
            <div>
              <div className="flex items-center gap-1">
                ...
                <img
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    window.open(`https://pump.fun/token/${token.address}`, '_blank');
                  }}
                />
              </div>
              <div className="text-xs text-foreground-secondary">{formatTimeAgo(token.createdAt)}</div>
            </div>
          </div>
